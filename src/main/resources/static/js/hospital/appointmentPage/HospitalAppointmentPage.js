...
/**
 * Load doctors for the current hospital
 */
function loadDoctors() {
    console.log('Loading doctors for current hospital...');
    ...
    .then(data => {
        ...
        if (data.success && data.doctors) {
            ...
            showNotification('⚠️ No doctors found for this hospital', 'warning');
        }
    })
    .catch(error => {
...
function populateDoctorDropdown(doctors) {
    ...
    if (doctors && doctors.length > 0) {
        ...
        option.disabled = true;
        doctorSelect.appendChild(option);
        console.log('No doctors available for this hospital');
    }
}
...
function cancelSchedule(scheduleId) {
    ...
    // Make API call
    fetch(`/hospital/cancel-schedule/${scheduleId}`, {
        method: 'DELETE',
        headers: {
            'Content-Type': 'application/json',
            'X-Requested-With': 'XMLHttpRequest'
...
/**
 * Load appointments for the current hospital
 */
function loadAppointments() {
    console.log('Loading appointments for current hospital...');
    
    fetch('/hospital/appointments-data', {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
            'X-Requested-With': 'XMLHttpRequest'
...
function cancelAppointment(appointmentId) {
    ...
    try {
        ...
        // Make API call
        fetch(`/hospital/cancel-appointment/${appointmentId}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                'X-Requested-With': 'XMLHttpRequest'
...
function reverseAppointment(appointmentId) {
    ...
    try {
        ...
        // Make API call
        fetch(`/hospital/reverse-appointment/${appointmentId}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-Requested-With': 'XMLHttpRequest'
