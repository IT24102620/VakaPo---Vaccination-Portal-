...
public class DoctorPasswordController {
    ...
    public ResponseEntity<?> changePassword(@RequestBody PasswordChangeRequest request) {
        try {
            ...
            log.info("Doctor password change request from: {}", userEmail);
            ...
            if (!"Doctor".equalsIgnoreCase(user.getRole())) {
                ...
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                        .body("{\"success\": false, \"error\": \"Access denied - doctors only\"}");
            }
            ...
            log.info("Doctor {} password changed successfully", userEmail);

            return ResponseEntity.ok().body("{\"success\": true, \"message\": \"Password changed successfully\"}");

        } catch (Exception e) {
            log.error("Error changing doctor password: {}", e.getMessage(), e);
