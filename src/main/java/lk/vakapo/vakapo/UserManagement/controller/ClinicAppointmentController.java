...
public class ClinicAppointmentController {
    ...
    /**
     * Display clinic appointment page
     */
    @GetMapping("/appointments")
    public String clinicAppointments(Model model) {
        try {
            // Get current authenticated user
            Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
            String userEmail = authentication.getName();
            log.info("Loading appointment page for clinic: {}", userEmail);
            ...
            if (clinicOpt.isPresent()) {
                ...
                // Get vaccination schedules for this clinic
                List<VaccinationSchedule> schedules = vaccinationScheduleService.getSchedulesByInstitution(clinic.getId(), "Clinic");
                
                // Get appointments for this clinic
                List<Appointment> appointments = appointmentService.getAppointmentsByInstitution(clinic.getId().toString(), "Clinic");
                ...
                log.info("Clinic appointment page loaded successfully for: {} with {} schedules and {} appointments", clinic.getUsername(), schedules.size(), appointments.size());
                return "clinic/appointmentPage/HospitalAppointmentPage";
            } else {
                log.error("Clinic not found for email: {}", userEmail);
                return "error/404";
            }
        } catch (Exception e) {
            log.error("Error loading clinic appointment page: {}", e.getMessage(), e);
            return "error/500";
        }
    }
    ...
    public ResponseEntity<?> getDoctors() {
        try {
            log.info("Fetching doctors for current clinic...");
            ...
            Clinic clinic = clinicOpt.get();
            log.info("Clinic found: {} with ID: {}", clinic.getUsername(), clinic.getId());
            
            // Get doctors for this clinic
            List<Staff> doctors = staffService.getStaffByRole(clinic.getId().toString(), "Clinic", "Doctor");
            log.info("Found {} doctors for clinic: {}", doctors.size(), clinic.getUsername());
    ...
    public ResponseEntity<?> testSchedule() {
        try {
            ...
            return ResponseEntity.ok().body("{\"success\": true, \"message\": \"Test successful\", \"clinicId\": \"" + clinic.getId() + "\", \"clinicName\": \"" + clinic.getUsername() + "\"}");
    ...
    public ResponseEntity<?> addSchedule(@RequestBody VaccinationScheduleRequest request) {
        try {
            ...
            schedule.setInstitutionType("Clinic");
            schedule.setInstitutionId(clinic.getId().toString());
    ...
    /**
     * Get appointments for the current clinic
     */
    @GetMapping("/appointments-data")
    @ResponseBody
    public ResponseEntity<?> getAppointments() {
        try {
            log.info("Fetching appointments for current clinic...");
            ...
            // Get appointments for this clinic
            List<Appointment> appointments = appointmentService.getAppointmentsByInstitution(clinic.getId().toString(), "Clinic");
            log.info("Found {} appointments for clinic: {}", appointments.size(), clinic.getUsername());
    ...
    public ResponseEntity<?> cancelAppointment(@PathVariable Long appointmentId) {
        try {
            ...
            if (!appointment.getInstitutionId().equals(clinic.getId().toString()) || !appointment.getInstitutionType().equals("Clinic")) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                        .body("{\"success\": false, \"error\": \"Appointment does not belong to this clinic\"}");
            }
            
            // Cancel the appointment
            boolean cancelled = appointmentService.cancelAppointmentByHospital(appointmentId, clinic.getId().toString());
    ...
    public ResponseEntity<?> reverseAppointment(@PathVariable Long appointmentId) {
        try {
            ...
            boolean reversed = appointmentService.reverseAppointment(appointmentId, clinic.getId().toString());
    ...
    public ResponseEntity<?> cancelSchedule(@PathVariable Long scheduleId) {
        try {
            ...
            if (!schedule.getInstitutionId().equals(clinic.getId()) || !schedule.getInstitutionType().equals("Clinic")) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                        .body("{\"success\": false, \"error\": \"Schedule does not belong to this clinic\"}");
            }
